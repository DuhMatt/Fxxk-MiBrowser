...
这是一个给 HyperOS / MIUI 用的 LSPosed 小模块。它处理的是小米系统里一个挺烦人的跳转习惯：系统明明已经设置了默认浏览器，小米互传、小爱识屏这类系统功能拿到网页链接后，还是会把链接塞给小米浏览器。小米浏览器如果已经卸载或禁用，还可能直接跳到小米应用商店的下载页。
...
模块会尽量在这个跳转发生前找回原始网页链接，然后交给系统默认浏览器处理。

从 `1.1` 开始，它会处理 Wi-Fi 详情页里的“管理小米路由”。如果设置页把路由器后台地址强制丢给小米浏览器，比如 `http://192.168.1.1`，模块会改用默认浏览器打开。

从 `1.2` 开始，它也会处理小爱识屏 / 超级小爱识别出来的链接。如果小爱把链接改成小米浏览器或小米应用商店下载页，模块会从识屏结果对象里把刚识别到的网页链接找回来，再交给默认浏览器。
...
- 修复小爱识屏 / 超级小爱识别网页链接后仍强制走小米浏览器的问题。
...
- 把原来的 MiShare 专用 URL 缓存扩展成 Xiaomi 系统组件通用缓存。
- 当小爱识屏链路已经变成 `mimarket://details?id=com.android.browser` 时，尝试从识屏结果对象里恢复原始 URL，再打开默认浏览器。
...
小爱识屏场景里，实测能从小米应用商店下载页恢复出识别到的链接：
...
This is a small LSPosed module for HyperOS / MIUI. It fixes a very specific Xiaomi annoyance: Mi Share, XiaoAi Screen Recognition, and a few other system features may force web links into Xiaomi Browser even when another default browser is already set. If Xiaomi Browser has been removed or disabled, the same path may end up on Xiaomi Market's browser download page.
...
The module tries to recover the original web URL before that fallback wins, then hands the link back to the system default browser.
...
Starting with `1.2`, it also handles XiaoAi / Super XiaoAi screen-recognition links. If the recognized URL gets rewritten to Xiaomi Browser or Xiaomi Market, the module restores the URL from the screen-recognition result object and opens it with the default browser.
...
- Fixes XiaoAi / Super XiaoAi screen-recognition links that are forced through Xiaomi Browser.
...
- When the XiaoAi path has already become `mimarket://details?id=com.android.browser`, tries to restore the original URL from the screen-recognition result object and open the default browser.
