...
这是一个给 HyperOS / MIUI 用的 LSPosed 小模块，主要解决一个很烦人的问题：系统明明有默认浏览器，小米互传收到网页链接时却还是想把链接交给小米浏览器；如果小米浏览器被卸载或禁用，还可能跳到小米应用商店的下载页。
